...
def aufg3():
	...
	plt.savefig('rnduniform.png',dpi=300)
	#plt.show()
	plt.close()

	e = np.random.exponential(scale=1.0, size=1000)
	count_e, bins_e, ignored_e = plt.hist(e, 15, normed=True,rasterized=True)
	plt.savefig('rndexp.png',dpi=300)
	#plt.show()	
	plt.close()
	
	r = cauchy.rvs( size=1000)
	count_r, bins_r, ignored_r = plt.hist(r, bins=np.linspace (-10. , 10., 50), normed=True,rasterized=True)
	plt.savefig('rndcauchy.png',dpi=300)
	#plt.show()
	plt.close()
	
	data = np.load("empirisches_histogramm.npy")
	plt.hist(data['bin_mid'], bins=np.linspace (0. , 1., 50) , weights =data['hist'],rasterized=True)
	#plt.show()
	plt.close()
